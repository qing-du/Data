...
epc_gas = economics.annuity(capex=500, n=30, wacc=0.052)
...
bcoal_new = solph.Bus(label = "coal_new")

blignite_old = solph.Bus(label = "lignite_old")

blignite_new = solph.Bus(label = "lignite_new")
...
energysystem.add(bcoal_old, bcoal_new, blignite_old, blignite_new, bgas, bel)
...
# source coal new
energysystem.add(solph.Source(label='coal_new', outputs={bcoal_new: solph.Flow(
    nominal_value=800, variable_costs = 9.6)}))
...
# source lignite new
energysystem.add(solph.Source(label='lignite_new', outputs={bcoal_new: solph.Flow(
    nominal_value=1000, variable_costs = 1.8)}))
...
energysystem.add(solph.Source(label='wind_on', outputs={bel: solph.Flow(fixed=True, 
        actual_value=data['wind_on'], investment = solph.Investment(ep_costs=epc_wind_on)))})

# source wind offshore
energysystem.add(solph.Source(label='wind_off', outputs={bel: solph.Flow(fixed=True, 
        actual_value=data['wind_off'], investment = solph.Investment(ep_costs=epc_wind_off)))})

# source pv
energysystem.add(solph.Source(label='pv', outputs={bel: solph.Flow(fixed=True, 
        actual_value=data['pv'], investment = solph.Investment(ep_costs=epc_pv)))})
...
energysystem.add(solph.Transformer(
    ...
    inputs={blignite_old: solph.Flow()},
...
energysystem.add(solph.Transformer(
    ...
    inputs={blignite_new: solph.Flow()},
